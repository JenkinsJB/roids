...
| OpenCV library | Apache-2.0 | ✓ Compatible (dynamic linking) |
...
- *Video Processing:* opencv crate or ffmpeg-next for video frame extraction
...
** Milestone 4: Video Support (Week 7-8)
...
| Video codec support issues    | Medium     | Medium | Test opencv vs ffmpeg crates early      |
...
| Performance with large videos | Low        | Medium | Rust performance should handle well     |
...
description = "Region Of Interest Designation System - A tool for annotating images and videos"
...
*** OpenCV/FFmpeg Considerations

- The opencv Rust crate (MIT) wraps the OpenCV C++ library (Apache-2.0)
- Apache-2.0 is compatible with BSD 3-Clause when dynamically linked
- If using ffmpeg-next, note that FFmpeg itself may be LGPL or GPL depending on build configuration
- For maximum permissiveness, ensure FFmpeg is built with LGPL-compatible options only
- Document runtime dependencies in README
